...
## Step 12 – 브랜드 & UI 리프레시 (예정)

1. **브랜드 방향 정의**  
   - 기존 UI 감사(색상 대비, 여백, 타이포) 결과 정리, 브랜드 키워드·톤 앤 매너 확정.  
   - 다크/하이콘트라스트 모드 지원 여부와 범위 결정.
2. **디자인 시스템 업데이트**  
   - Figma 스타일 가이드 ↔ Tailwind Theme/Tokens 매핑, 컴포넌트 영향도 분석.  
   - 아이콘/일러스트 자산 확보, 전역 스타일 토큰 문서화.
3. **단계적 적용**  
   - `src/components/ui` 공통 요소부터 리뉴얼, 핵심 페이지 → 부가 페이지 순으로 롤아웃.  
   - 테마 전환 토글 및 상태 저장(예: localStorage) 구현.
4. **검증·배포**  
   - 디자인 QA, 사용자 인터뷰 혹은 제한적 베타로 피드백 수집.  
   - README/디자인 문서 업데이트, QA 프로세스에 시각 검수 단계 추가 후 전면 배포.
