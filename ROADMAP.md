...
## Step 17 – 투표·인증 관련 페이지 UI 일원화 및 수정 (진행 중)
...
- ⏳ **인증 UI 정비 계획**: `/signin`, `/signup`을 투표 페이지 톤에 맞춘 상단 메타/카드 레이아웃으로 개편하고 CTA 정렬·비로그인 안내 문구를 표준화합니다(버튼 그리드·상태 배지와 색상 토큰 재사용).
- ✅ **표시 지표/구성 합의**:
...
18. **Step 17** – ⏳ 진행중: 투표·인증 관련 페이지 UI 일원화 및 수정 (목록 그리드·읽기 전용 전환, 참여 상태 UI 재구성, 인증 UI 계획 추가).
