...
## Step 20 – 비공개 투표 언리스트드 링크 + 정원 제한 (진행 중)
...
- ⏳ **DB/RPC 실행 필요**: `references/QUERY.md` Step 20 블록을 Supabase SQL Editor에서 실행해 `polls.max_voters` 컬럼 추가와 `create_new_poll`/`can_access_poll`/`get_poll_with_user_status`/`increment_vote` 확장을 적용해야 합니다.
...
21. **Step 20** – ⏳ 진행 중: 비공개 투표 언리스트드 링크 + 정원 제한.
