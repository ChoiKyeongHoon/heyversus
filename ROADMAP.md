...
- ✅ **초기 상태 정확도**: `/polls` 서버 prefetch를 세션 클라이언트로 수행해 즐겨찾기/투표 상태가 첫 렌더에서 일치하도록 Hydration합니다.
- ✅ **테스트 보강**: `/api/polls` GET/POST, `/api/polls/[id]` 404/403, `usePollVote` 오류 롤백, 즐겨찾기 토글 성공/권한 실패 등 핵심 플로우를 Jest + RTL로 커버했습니다.
- 📄 **참고 문서**: `references/ISSUE_REVIEW_2024-11.md`.

## Step 19 – 투표 이미지 업로드 기능 (완료)

- ✅ **계획 수립**: `references/STEP19_IMAGE_UPLOAD_PLAN.md`에 스토리지/RLS, RPC, 업로드 플로우, 검증·보안, 테스트 시나리오를 정리했습니다.
- ✅ **스토리지 정책 확장**: `poll_images` 버킷(비공개, 10MB, JPEG/PNG/WebP) 및 서비스 롤/소유자 전용 RLS 정책을 추가하고, 5분 만료 서명 URL 기반 접근을 전제로 했습니다. (SQL: `references/QUERY.md` Step 19 블록)
- ✅ **RPC & 스키마 업데이트**: `poll_options.image_url` 컬럼을 보강하고 `create_new_poll`가 `option_image_urls` 배열을 받아 길이 검증 후 함께 저장합니다.
- ✅ **업로드 API**: `/api/polls/images`에서 서명 업로드 URL 발급(10MB/확장자 검증) 및 클린업(DELETE) 엔드포인트를 추가했습니다.
- ✅ **프론트엔드 UX**: `/create-poll`에서 옵션별 이미지 업로드/미리보기/삭제, 업로드 상태/에러 노출, 10MB/확장자 가드를 적용하고 업로드 진행 중 제출 버튼을 비활성화했습니다.
- ✅ **응답 서명 처리**: Poll 서비스 계층이 `poll_images` 객체 경로를 5분 만료 서명 URL로 변환해 목록/상세/대표/즐겨찾기/내 투표에서 안전하게 렌더링합니다.

## Step 20 – 비공개 투표 언리스트드 링크 + 정원 제한 (✅ 완료)

- ✅ **범위 확정**: 비공개 투표는 목록에 숨기고, 로그인 사용자는 링크로 접근·투표 가능하도록 모델을 단순화했습니다.
- ✅ **프론트엔드 UI/UX**: `/create-poll`에서 비공개 선택 시 `maxVoters`(선착순 정원) 입력을 노출하고, `/poll/[id]`에서 “현재 X / 최대 N명” 표시 및 정원 마감 시 결과-only UX를 추가했습니다.
- ✅ **DB/RPC 적용**: `references/QUERY.md` Step 20 블록 실행으로 `polls.max_voters` 컬럼 추가와 `create_new_poll`/`can_access_poll`/`get_poll_with_user_status`/`increment_vote` 확장을 적용했습니다.

## Step 21 – 관리자 운영 대시보드 (✅ 완료)

- ✅ **계획 수립**: `references/STEP21_ADMIN_DASHBOARD_PLAN.md`에 관리자 권한 모델(A안), 신고 테이블/플로우, 지표 JSONB 설계, 관리자 액션/감사 로그, `/admin` UI 구조를 정리했습니다.
- ✅ `/admin` 보호 라우트를 추가하고 Supabase RLS 및 역할 기반 인증(관리자 전용)을 적용.
- ✅ 신고된 투표 목록, 기본 운영 지표(기간 필터) 등을 한눈에 확인할 수 있는 카드/리스트 UI 구성.
- ✅ 관리자가 투표 공개/비공개 전환, 삭제, 대표 지정/해제를 수행할 수 있는 액션과 `admin_audit_logs` 감사 로그 기록 추가.
- ✅ **투표 관리(검색/선택지 이미지)**: `/admin`에서 투표 목록 검색/필터/페이지네이션과 선택지(투표 대상) 이미지 편집(업로드/외부 URL)을 제공하고, 모든 선택지에 이미지가 있어야 대표 투표로 지정 가능하도록 가드(서버+UI)를 추가.
- ✅ **대표 투표 단일화**: 대표 지정 시 기존 대표 투표는 자동 해제되며, DB 제약으로 `is_featured=true`가 복수로 유지되지 않도록 방지합니다.
- ✅ `/admin` 접속/부트스트랩 가이드 문서화: `references/STEP21_ADMIN_DASHBOARD_PLAN.md`에 접속 URL, role 부여 SQL, 오류 체크리스트를 추가.
- ⏳ 분석 이벤트, 이메일 알림과 연계한 워크플로 문서화.
...
- ✅ **투표 상세 모바일 CTA 겹침 해결**: 모바일(`sm` 미만)에서 투표 목록 버튼이 랜덤 투표/투표하기 버튼과 겹치지 않도록 라벨을 숨겨(화살표만) 폭을 줄이고, absolute 배치 의도(좌측 고정 + 중앙 대칭)를 유지했습니다.
- ✅ **랜덤 투표 룰렛 포인터 방향/위치 수정**: 랜덤 투표 모달에서 룰렛 상단 포인터가 반대로(위쪽) 향하던 문제를 수정해, 원통(룰렛) 방향으로 가리키도록 정방향(아래쪽) 삼각형으로 표시하고 포인터가 살짝 떠 보이도록(약 20%만 겹침) 위치를 조정했습니다.
- ✅ **랜덤 투표 룰렛 조각 번호 표기**: 룰렛 원판 내부에 옵션 번호(1..N)를 표시해, 스핀 중에도 아래 옵션 리스트의 `#n`과 빠르게 매칭할 수 있도록 했습니다. (배지 없이 텍스트 + 그림자)
...
- 📄 **캐시 동기화 문서화**: `references/STEP22_CACHE_SYNC_PLAN.md`에 캐시 태그/경로와 재검증 순서를 정리하고, 향후 테스트/QA 체크리스트를 기록했습니다.
...
- ⏳ **`profiles.role` 노출 최소화(옵션 결정)**: (A) 공개용 `public_profiles` view/RPC로 role 제외, (B) roles 분리 테이블(`profile_roles`) + RLS로 admin만 조회.
- ⏳ **관리자 접근 강화(방어 심화)**: `middleware`로 `/admin`, `/api/admin/*` 선차단(세션/role), 관리자 API 레이트 리밋, 관리자 액션/권한 실패 이벤트 알림(옵션: IP allowlist).
- ✅ **신고 UX 결정(리스트 노출 보류)**: `/polls`에서 신고 버튼을 즉시 노출하는 것은 대량 신고 트롤링 리스크가 있어 보류하고, 현재처럼 `/poll/[id]` 상세에서만 신고를 제공합니다. 필요 시 `create_report` 중복 방지/레이트 리밋 등 방어를 추가한 뒤 리스트 UX를 재검토합니다. (문서: `references/STEP22_REPORT_UX_PLAN.md`)
- ⏳ **지표 모니터링 & 알림**: 점수 변동/편향/이상 행태 모니터링, 알림 임계값 정의, 운영 가이드 문서화.
- 💤 **후속 보류**: 초대 화이트리스트/토큰/이메일 초대, 수동·예약 마감, 정원 증설/재오픈 등 운영 기능은 Step 22에서 재논의합니다.
...
20. **Step 19** – ✅ 완료: 투표 이미지 업로드(서명 URL/10MB 제한) 기능.
21. **Step 20** – ✅ 완료: 비공개 투표 언리스트드 링크 + 정원 제한.
22. **Step 21** – ✅ 완료: 관리자 운영 대시보드.
23. **Step 22** – ⏳ 예정: 지속적 개선 (랭킹 알고리즘/실시간 파이프라인/리더보드 UX/모니터링, 썸네일/리사이즈, 고아 객체 정리, CDN/Edge 전략).
