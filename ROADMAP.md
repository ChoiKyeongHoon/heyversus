...
- ## Step 16 – 돌림판(랜덤 옵션 선택) 기능 (예정)

> 상세 설계는 `references/RANDOM_ROULETTE_PLAN.md`를 참조하세요.

- ⏳ **옵션 돌림판 도입**: 투표 상세 옵션 영역에 “돌림판으로 선택하기” 버튼을 추가해 옵션 중 하나를 무작위 하이라이트합니다. 자동 투표는 하지 않고, 사용자 확인 후 수동 투표를 유지합니다.
- ⏳ **애니메이션 & 피드백**: 회전/감속 애니메이션과 당첨 옵션 강조 UI(배지/토스트)를 제공해 참여 재미를 높입니다. 선택된 옵션은 일시적으로 비활성화해 중복 방지를 안내합니다.
- ⏳ **중복/신뢰성 처리**: 한 세션 내 중복 회전을 제한하거나 쿨다운을 적용하고, 이미 투표한 상태/마감 투표에서는 돌림판 버튼을 숨깁니다. 반응형(모바일 우선)으로 터치 영역 44px 이상 유지합니다.
- ⏳ **UI/UX 연동**: `/poll/[id]` 내부 모달로 구현하여 이동 없이 옵션 선택 경험을 제공하고, 추천 없음 시 EmptyState를 노출합니다.
- ⏳ **실험 및 로깅**: 필요 시 노출/클릭/투표 전환 로그를 수집해 품질을 분석하고, 기본 구현에서는 로그를 생략한 뒤 후속 단계에서 도입합니다.
