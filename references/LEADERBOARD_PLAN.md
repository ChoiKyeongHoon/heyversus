...
## Scope & Guardrails
- 목표: 점수 공식/랭킹을 재정의해 활동을 반영하고, `/score` UX를 확장한다. (현재 단계: 감가 미적용, 단순 합산)
...
- 감가: 현재 미적용(향후 필요 시 half-life 기반으로 확장 가능).
...
## RPC / Batch
- `refresh_profile_scores(p_limit, p_offset)`: 일일 배치 합산(감가 없음) + 주간 캡은 후속 확장 예정.
