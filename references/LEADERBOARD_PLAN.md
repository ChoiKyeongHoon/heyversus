...
## Scope & Guardrails
- 목표: 점수 공식/랭킹을 재정의해 최근 활동을 반영하고, `/score` UX를 확장한다.
...
- 성능/비용: 감가·점수 계산은 배치 중심, 실시간 반영은 최소화(선택적). 인덱스·페이지네이션을 전제로 설계한다.
...
- 감가: 지표별 half-life 차등 — 투표/즐겨찾기 7일, 공유/이벤트성 3일. 배치 시 `weight * exp(-age/half_life)`로 재계산.
- 비정상 패턴 가드: 레이트 리밋은 점수 계산과 분리(IP/세션 레벨), 로그 기반 모니터링으로 급증 탐지.

## Data Model
- `profile_scores` (PK: user_id): `score`(DECIMAL), `last_activity_at`, `decay_applied_at`, `raw_points_cache`(옵션), `weekly_cap_hit`(bool).
...
## RPC / Batch
- `refresh_profile_scores(p_limit, p_offset)`: 일일 배치 감가 + 주간 캡 반영, 부분 처리 가능.
...
## QA & Ops
- QA: 감가/상한 동작, 중복 차단, 비로그인 조회, 다크/라이트 테마, 모바일 1열/데스크톱 2열 레이아웃.
