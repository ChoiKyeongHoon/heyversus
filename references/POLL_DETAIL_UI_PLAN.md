...
## 현재 상태 (Step 17 진행 중)
- `PollClient`가 투표/룰렛 로직과 UI를 모두 포함하고 있어 카드 추출 전 단계다. (`src/app/poll/[id]/PollClient.tsx`)
- 상단 메트릭(총 투표 수/상태/남은 시간/선두 옵션)과 즐겨찾기 아이콘 정렬은 PollsHero 톤에 맞춰 일부 정리됨.
- 랜덤 돌림판 모달과 즐겨찾기 토글이 상세 페이지에 통합되어 UX 요구사항은 충족하지만, 레이아웃/스타일 공통화는 미완료다.
...
## 다음 우선 작업
- `PollDetailCard`를 생성해 렌더를 위임하고, `PollClient`는 상태/핸들러만 관리하도록 분리한다.
- 옵션 렌더 스타일을 PollCard와 동일한 토큰/여백으로 맞추고, CTA 슬롯(상태 배너+버튼)을 고정된 위치로 정렬한다.
- 룰렛 모달/선택 하이라이트를 별도 하위 컴포넌트로 쪼개어 테스트/가독성을 높인다.
